...
class BatchMolGraph:
    ...
    def get_ggnn_features(self):
        max_atoms = np.max([mol_graph.n_atoms for mol_graph in self.mol_graphs])
        out_size = max_atoms
        batch_atom_array, batch_adj_array = zip(*[mol_graph.ggnn_features(max_atoms, out_size) for mol_graph in self.mol_graphs])
        batch_atom_tensor = torch.LongTensor(batch_atom_array)
        batch_adj_tensor = torch.FloatTensor(batch_adj_array)
        # print(batch_atom_tensor, batch_adj_tensor)
        return batch_atom_tensor, batch_adj_tensor
