...
class GCNEncoderWithFeatures(nn.Module):
    def __init__(self, args: Namespace, num_features: int, features_nonzero: int,
                 dropout: float = 0.1, bias: bool = False,
                 sparse: bool = True):#num_features: self.num_features + self.args.input_features_size
        ...
        self.sparse = sparse
        GC = SparseGraphConvolution if sparse else GraphConvolution
        self.gc_input = GC(in_features=num_features, out_features=args.gcn_hidden1, bias=bias)# num_features*300
        self.gc_hidden1 = GC(in_features=args.gcn_hidden1, out_features=args.gcn_hidden2, bias=bias)# 300*281
        self.gc_hidden2 = GC(in_features=args.gcn_hidden2, out_features=args.gcn_hidden3, bias=bias)# 281*264
    ...
    def forward(self, features: torch.Tensor, adj: torch.sparse.FloatTensor) -> torch.Tensor:
        ...
        embeddings = F.relu(self.trans_h2(torch.cat([self.gc_hidden2(hidden2, adj), features], dim=1)))#embedding维度是264
        return embeddings